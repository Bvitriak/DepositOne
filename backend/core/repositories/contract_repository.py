...
def build_where(search, status):
    ...
    if search:
        ...
        like = "%" + search + "%"
        parameters = parameters + [like, like, like, like, like, like]
    if status:
        conditions.append("ct.signing_status = %s")
...
def list_contracts(connection, search, status, sort, order, limit, offset):
...
def count_contracts(connection, search, status):
    where, parameters = build_where(search, status)
