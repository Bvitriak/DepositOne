...
async function loadDepositors() {
  ...
  let response;
  try {
    response = await fetch("/api/depositors" + query, { headers: { Authorization: "Bearer " + token } });
  } catch {
  ...
  page = data.page;
  document.getElementById("depositorList").innerHTML = depositorList(data);
}

document.getElementById("searchArea").innerHTML = depositorSearchBar();
...
const listElement = document.getElementById("depositorList");
